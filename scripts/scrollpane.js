...
content.addEventListener('scroll', function (e) {
  ...
  scroll.style.left = ((container.clientWidth - scroll.clientWidth) / 100) * percentScrolled + "px";
  scrollLower.style.left = ((container.clientWidth - scrollLower.clientWidth) / 100) * percentScrolled + "px";
});
...
function addScrollbarEvent(scrollbar) {
  scrollbar.addEventListener('mousedown', function (start) {
    ...
    var onMove = function (end) {
      ...
      scrollbar.style.left = Math.min(container.clientWidth - scrollbar.clientWidth, Math.max(0, y + delta)) + 'px';

      // percent of scrollbar moved
      var percentScrolled = 100 / ((container.clientWidth - scrollbar.clientWidth) / scrollbar.offsetLeft);
      content.scrollLeft = ((content.scrollWidth - content.clientWidth) / 100) * percentScrolled;
    };
    console.log(start);
